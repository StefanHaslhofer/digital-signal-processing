...
%#########################################################################
% Assignment 5, Aufgabe 1 : FIR Tiefpassentw�rfe
%#########################################################################
...
Delta_2 = 10^(-45/20); 			            % stopband tolerance (20 log(Delta_2/1) = -45 dB
...
nv = -N/2:N/2;
h_ideal = (Omega_0/pi).*sinc(nv*Omega_0);
...
h_hw = h_ideal.*w;                        % h_hw (h_hamming-window)
...
N=length(h_pm)+1;                    %Falls sich herausstellt, dass das Toleranzschema mit der in
%                          %"firpmord" berechneten Filterordnung noch nicht eingehalten wird
%                          %erh�hen Sie hier die Filterordnung
